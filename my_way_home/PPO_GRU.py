...
def main():
    ...
    # 更改为使用PPO
    model = PPO(CustomGRUPolicy, env, device=device, verbose=1)
    ...
    for episode in range(episodes):
        ...
        print(
            f"PPO Agent: Episode {episode + 1} - Duration: {episode_durations[-1]:.2f} seconds - Total Reward: {episode_rewards[-1]}")
    ...
    model_filename = f"PPO_GRU_task1_{formatted_date_time}.zip"
    nn_filename = f"PPO_GRU_task_NN_{formatted_date_time}.pth"
